...
router.post("/registreer", function (req, res, next) {
    if (!req.body.username || !req.body.password || !req.body.email) {
        return res
            .status(400)
            .json({
                message: "U heeft een veld open gelaten. Vul deze aub in."
            });
    }
    User.find({
            ...
            User.find({
                ...
                if (result.length) {
                    return res
                        .status(400)
                        .json({
                            message: "Gebruikersnaam reeds in gebruik"
                        });
                } else {
                    let user = new User();
                ...
                function (err, obj) {
                    if (err || obj == null) {
                        return res.status(417).json({
                            message: "Er liep iets mis met het uitvoeren van deze beveiligde actie."
                        });
                    }
                    return res.json({
...
router.get("/getPreferences", function (req, res, next) {
    ...
    User.findOne({
        ...
        function (err, obj) {
            ...
            return res.json({
                soortenMeetings: obj.soortenMeetings
            });
        }
    );
});
...
router.post("/changePreferences", authentication, function (req, res, next) {
    ...
    User.findOneAndUpdate({
            ...
            $set: {
                soortenMeetings: req.body.soortenMeetings
            }
        },
